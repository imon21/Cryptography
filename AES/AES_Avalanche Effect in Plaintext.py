...
plaintext2 = open("plaintext2.txt","r").read()
key = []
pp2 = []
pp = []
for i in range(16):
...
addRound2 = addRoundKey(plain2,key[:16])
for i in range(1,11):
    ...
    if i != 10:
        ...
        plain2 = mixcolm2
        addRound2 = addRoundKey(plain2,key[(i*16):((i+1)*16)])
    else:
        addRound = addRoundKey(shftarray,key[(i*16):((i+1)*16)])
        addRound2 = addRoundKey(shftarray2,key[(i*16):((i+1)*16)])
    print("Avalanche Effect in plaintext: ")
